...
public class AddArtikelPresenter implements AddArtikelContract.Presenter, AddArtikelContract.Listener {
    AddArtikelContract.View mView;
    AddArtikelContract.Repository mRepo;

    public AddArtikelPresenter(AddArtikelContract.View mView) {
        this.mView = mView;
        mRepo = new AddArtikelRepository(this);
    }
    ...
    @Override
    public void headerImage(String edt, Uri uri) {
        if (uri != null) {
            mRepo.upImgHeader(uri);
        }
        if (!edt.isEmpty()) {
            Log.d("par1", edt);
